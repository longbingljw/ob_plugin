...
/**
 * Batch Thai Segmenter for processing text files
 * Reads input file line by line, segments each line, and writes results to timestamped output file
 */
public class BatchThaiSegmenter {
    private ThaiSegmenter segmenter;  // Changed from RealThaiSegmenter
    private final String RESULTS_DIR = "th_results";

    public BatchThaiSegmenter() {
        this.segmenter = new ThaiSegmenter();  // Changed from RealThaiSegmenter
    ...
    /**
     * Generate timestamped filename for Thai segmentation results
     * Format: th_YYYYMMDD_HHMMSS.txt
     */
    private String generateTimestampedFilename() {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss");
        String timestamp = LocalDateTime.now().format(formatter);
        return "th_" + timestamp + ".txt";
    }
     ...
     * Usage: java BatchThaiSegmenter <input_file.txt>
     */
    public static void main(String[] args) {
        if (args.length != 1) {
            System.out.println("Usage: java BatchThaiSegmenter <input_file.txt>");
            System.out.println("Example: java BatchThaiSegmenter thai_text.txt");
            return;
        }

        String inputFile = args[0];
        BatchThaiSegmenter batchProcessor = new BatchThaiSegmenter();
