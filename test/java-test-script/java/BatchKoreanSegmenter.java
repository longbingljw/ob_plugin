...
/**
 * Batch Korean Segmenter for processing text files
 * Reads input file line by line, segments each line, and writes results to timestamped output file
 */
public class BatchKoreanSegmenter {
    private KoreanSegmenter segmenter;
    private final String RESULTS_DIR = "ko_results";

    public BatchKoreanSegmenter() {
        this.segmenter = new KoreanSegmenter();
    ...
    /**
     * Generate timestamped filename for Korean segmentation results
     * Format: ko_YYYYMMDD_HHMMSS.txt
     */
    private String generateTimestampedFilename() {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss");
        String timestamp = LocalDateTime.now().format(formatter);
        return "ko_" + timestamp + ".txt";
    }
     ...
     * Usage: java BatchKoreanSegmenter <input_file.txt>
     */
    public static void main(String[] args) {
        if (args.length != 1) {
            System.out.println("Usage: java BatchKoreanSegmenter <input_file.txt>");
            System.out.println("Example: java BatchKoreanSegmenter korean_text.txt");
            return;
        }

        String inputFile = args[0];
        BatchKoreanSegmenter batchProcessor = new BatchKoreanSegmenter();
