 ...
 * Experimental Japanese Fulltext Parser Plugin - Simplified Implementation
 */
...
SimplifiedJNIBridgeConfig::SimplifiedJNIBridgeConfig() 
    ...
    if (env_classpath && strlen(env_classpath) > 0) {
        ...
        // Default fallback paths with explicit JAR files
...
// SimplifiedJNIBridge implementation
SimplifiedJNIBridge::SimplifiedJNIBridge() 
    : plugin_name_("experimental_japanese_ftparser")
...
SimplifiedJNIBridge::~SimplifiedJNIBridge() {
    if (is_initialized_) {
        ...
        GlobalJVMManager::unregister_plugin(plugin_name_);
        is_initialized_ = false;
    }
}

int SimplifiedJNIBridge::initialize() {
    ...
    GlobalJVMManager::register_plugin(plugin_name_);
    
    // Create scoped JNI environment for initialization
    ScopedJNIEnvironment jni_env(plugin_name_, config_.java_class_path, 
...
int SimplifiedJNIBridge::segment(const std::string& text, std::vector<std::string>& tokens) {
    if (!is_initialized_) {
...
int SimplifiedJNIBridge::load_java_classes(JNIEnv* env) {
    if (!env) {
    ...
    if (!segmenter_class_ || JNIUtils::check_and_handle_exception(env, error_msg)) {
    ...
    if (!constructor_method_ || JNIUtils::check_and_handle_exception(env, error_msg)) {
    ...
    if (!segment_method_ || JNIUtils::check_and_handle_exception(env, error_msg)) {
...
int SimplifiedJNIBridge::do_segment(JNIEnv* env, const std::string& text, std::vector<std::string>& tokens) {
    if (!env) {
        set_error(OBP_PLUGIN_ERROR, "JNI environment is null");
        return OBP_PLUGIN_ERROR;
    }
    ...
    jstring jtext = JNIUtils::cpp_string_to_jstring(env, text);
    if (!jtext) {
    ...
    std::string error_msg;
    if (!local_segmenter || JNIUtils::check_and_handle_exception(env, error_msg)) {
    ...
    if (JNIUtils::check_and_handle_exception(env, error_msg)) {
    ...
    int ret = JNIUtils::jstring_array_to_cpp_vector(env, jresult, tokens);
    if (ret != 0) {
...
void SimplifiedJNIBridge::set_error(int code, const std::string& message) {
    last_error_.error_code = code;
    last_error_.error_message = message;
    std::cout << "[ERROR][SimplifiedJNIBridge] " << message << std::endl;
}

void SimplifiedJNIBridge::clear_error() {
    last_error_.error_code = OBP_SUCCESS;
    last_error_.error_message.clear();
    last_error_.java_exception.clear();
}

// SimplifiedJNIBridgeManager implementation
SimplifiedJNIBridgeManager& SimplifiedJNIBridgeManager::get_instance() {
    static SimplifiedJNIBridgeManager instance;
    return instance;
}

std::shared_ptr<SimplifiedJNIBridge> SimplifiedJNIBridgeManager::get_bridge() {
    std::lock_guard<std::mutex> lock(mutex_);
    if (!bridge_) {
        bridge_ = std::make_shared<SimplifiedJNIBridge>();
    }
    return bridge_;
}

int SimplifiedJNIBridgeManager::initialize() {
    auto bridge = get_bridge();
    return bridge ? bridge->initialize() : OBP_PLUGIN_ERROR;
}

// Plugin parser structure
struct ExperimentalJapaneseParser {
    std::vector<std::string> tokens;
    size_t current_token_index;
    
    ExperimentalJapaneseParser() : current_token_index(0) {}
};

// Plugin interface implementation
extern "C" {

int experimental_japanese_ftparser_init(ObPluginParamPtr param) {
    if (!param) {
    ...
    std::cout << "[INFO] Experimental Japanese FTParser plugin registered (JVM will be initialized on first use)" << std::endl;
    return OBP_SUCCESS;
}

int experimental_japanese_ftparser_deinit(ObPluginParamPtr param) {
    if (!param) {
        return OBP_INVALID_ARGUMENT;
    }
    
    std::cout << "[INFO] Experimental Japanese FTParser deinitialized" << std::endl;
    return OBP_SUCCESS;
}

int experimental_japanese_ftparser_scan_begin(ObPluginFTParserParamPtr param) {
    if (!param) {
        return OBP_INVALID_ARGUMENT;
    }
    
    // Lazy initialization: initialize JVM on first actual use
    auto& manager = SimplifiedJNIBridgeManager::get_instance();
    ...
    ExperimentalJapaneseParser* jp = new (std::nothrow) ExperimentalJapaneseParser();
    if (!jp) {
...
int experimental_japanese_ftparser_scan_end(ObPluginFTParserParamPtr param) {
    if (!param) {
        return OBP_INVALID_ARGUMENT;
    }
    
    ExperimentalJapaneseParser* jp = (ExperimentalJapaneseParser*)obp_ftparser_user_data(param);
    if (jp) {
        delete jp;
...
int experimental_japanese_ftparser_next_token(ObPluginFTParserParamPtr param, char **word, int64_t *word_len, int64_t *char_cnt, int64_t *word_freq) {
...
int experimental_japanese_ftparser_get_add_word_flag(uint64_t *flag) {
    if (!flag) {
        return OBP_INVALID_ARGUMENT;
    }
